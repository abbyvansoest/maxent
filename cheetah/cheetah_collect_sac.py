...
# python cheetah_collect_sac.py --env="HalfCheetah-v2" --exp_name=_discretize_autoencoder_6 --T=1000 --n=20 --l=2 --hid=300 --epochs=16 --episodes=30 --autoencode --autoencoder_reduce_dim=6
...
import plotting
from cheetah_soft_actor_critic import CheetahSoftActorCritic
...
def collect_avg_obs(env, policies, T, n=100):
    ...
    for iteration in range(n):        
        ...
        for t in range(T):
            action = np.zeros(shape=(1,cheetah_utils.action_dim))
            ...
            data.append(obs[:29])
            obs = cheetah_utils.get_state(env, obs)
    ...
    for it in range(N): 
        ...
        for iteration in range(n):
            ...
            if len(initial_state) > 0:
                qpos = initial_state[:len(cheetah_utils.qpos)]
            ...
            for t in range(T):
                ...
                if p_xy[tuple(cheetah_utils.discretize_state_2d(obs, norm, env))]  == 0:
                    cumulative_states_visited_xy += 1
                
                step = iteration*T + t
                states_visited_xy[step] += cumulative_states_visited_xy
                p_xy[tuple(cheetah_utils.discretize_state_2d(obs, norm, env))] += 1
...
def execute_one_rollout(policies, weights, env, start_obs, T, data, norm, wrapped=False):
    ...
    for t in range(T):
        ...
        # Count the cumulative number of new states visited as a function of t.
        obs, _, done, _ = env.step(action)
        obs = cheetah_utils.get_state(env, obs, wrapped)

        # if this is the first time you are seeing this state, increment.
        if p[tuple(cheetah_utils.discretize_state(obs, norm, env))] == 0:
        ...
        p[tuple(cheetah_utils.discretize_state(obs, norm, env))] += 1
        p_xy[tuple(cheetah_utils.discretize_state_2d(obs, norm, env))] += 1
...
def grad_ent(pt):
    ...
    eps = 1/np.sqrt(cheetah_utils.total_state_space)
    return 1/(pt + eps)

def init_state(env):    
    env.env.set_state(cheetah_utils.qpos, cheetah_utils.qvel)
...
def collect_entropy_policies(env, epochs, T, MODEL_DIR=''):
    ...
    running_avg_p = np.zeros(shape=(tuple(cheetah_utils.num_states)))
    running_avg_p_xy = np.zeros(shape=(tuple(cheetah_utils.num_states_2d)))
    running_avg_ent = 0
    running_avg_ent_xy = 0
    entropy_of_running_avg_p = 0.

    running_avg_p_baseline = np.zeros(shape=(tuple(cheetah_utils.num_states)))
    running_avg_p_baseline_xy = np.zeros(shape=(tuple(cheetah_utils.num_states_2d)))
    ...
    for t in range(10000):     
        ...
        prebuf.store(cheetah_utils.get_state(env, obs))
        if done:
            env.reset()
            done = False
    ...
    reward_fn = np.zeros(shape=(tuple(cheetah_utils.num_states)))
    ...
    for i in range(epochs):
        ...
        sac = CheetahSoftActorCritic(lambda : gym.make(args.env), reward_fn=reward_fn, xid=i+1,
        ...
        if args.autoencode:
            ...
            cheetah_utils.learn_encoding(train, test)
