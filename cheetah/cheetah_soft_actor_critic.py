...
class CheetahSoftActorCritic:
    ...
    def __init__(self, env_fn, reward_fn=[], actor_critic=core.mlp_actor_critic, 
        ...
        if self.learn_reduced:
            self.obs_dim = cheetah_utils.expected_state_dim
    ...
    def reward(self, env, r, o):
        ...
        tup = tuple(cheetah_utils.discretize_state(o, self.normalization_factors, env))
        return self.reward_fn[tup]

    def get_action(self, o, deterministic=False): 
        if self.learn_reduced:
            o = cheetah_utils.convert_obs(o)
        with self.graph.as_default():
    ...
    def test_agent(self, T, n=10, initial_state=[], normalization_factors=[], store_log=True, deterministic=True, reset=False):
        ...
        for j in range(n):
            ...
            o = cheetah_utils.get_state(self.test_env, o)
            while not(d or (ep_len == T)):
                ...
                p[tuple(cheetah_utils.discretize_state(o, normalization_factors, self.test_env))] += 1
                p_xy[tuple(cheetah_utils.discretize_state_2d(o, normalization_factors, self.test_env))] += 1
    ...
    def test_agent_random(self, T, normalization_factors=[], n=10):
        ...
        for j in range(n):
            ...
            while not(d or (ep_len == T)):
                ...
                if p[tuple(cheetah_utils.discretize_state(o, normalization_factors, self.test_env))] == 0:
                    cumulative_states_visited_baseline += 1
                states_visited_baseline.append(cumulative_states_visited_baseline)
                if p_xy[tuple(cheetah_utils.discretize_state_2d(o, normalization_factors, self.test_env))]  == 0:
    ...
    def soft_actor_critic(self, initial_state=[], steps_per_epoch=5000, epochs=100,
        ...
        with self.graph.as_default():
            ...
            o = cheetah_utils.get_state(self.env, o)
            ...
            for t in range(total_steps):
                ...
                o2 = cheetah_utils.get_state(self.env, o2)
                ...
                if self.learn_reduced:
                    self.replay_buffer.store(cheetah_utils.convert_obs(o), 
                                             a, r, cheetah_utils.convert_obs(o2), d)
                else:
...
if __name__ == '__main__':
    ...
    sac1 = CheetahSoftActorCritic(lambda : gym.make(args.env), 
