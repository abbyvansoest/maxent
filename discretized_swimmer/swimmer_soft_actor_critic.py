...
class SwimmerSoftActorCritic:
    ...
    def __init__(self, env_fn, reward_fn=[], actor_critic=core.mlp_actor_critic, 
        ...
        if self.learn_reduced:
            self.obs_dim = swimmer_utils.expected_state_dim
    ...
    def reward(self, env, r, o):
        ...
        tup = tuple(swimmer_utils.discretize_state(o, self.normalization_factors, env))
        return self.reward_fn[tup]

    def get_action(self, o, deterministic=False): 
        if self.learn_reduced:
            o = swimmer_utils.convert_obs(o)
        with self.graph.as_default():
    ...
    def test_agent(self, T, n=10, initial_state=[], normalization_factors=[], store_log=True, deterministic=True, reset=False):
        ...
        for j in range(n):
            ...
            while not(d or (ep_len == T)):
                ...
                p[tuple(swimmer_utils.discretize_state(o, normalization_factors, self.test_env))] += 1
                p_xy[tuple(swimmer_utils.discretize_state_2d(o, normalization_factors, self.test_env))] += 1
    ...
    def test_agent_random(self, T, normalization_factors=[], n=10):
        ...
        for j in range(n):
            ...
            while not(d or (ep_len == T)):
                ...
                if p[tuple(swimmer_utils.discretize_state(o, normalization_factors, self.test_env))] == 0:
                    cumulative_states_visited_baseline += 1
                states_visited_baseline.append(cumulative_states_visited_baseline)
                if p_xy[tuple(swimmer_utils.discretize_state_2d(o, normalization_factors, self.test_env))]  == 0:
    ...
    def soft_actor_critic(self, initial_state=[], steps_per_epoch=5000, epochs=100,
        ...
        with self.graph.as_default():
            ...
            for t in range(total_steps):
                ...
                if self.learn_reduced:
                    self.replay_buffer.store(swimmer_utils.convert_obs(o), 
                                             a, r, swimmer_utils.convert_obs(o2), d)
                else:
