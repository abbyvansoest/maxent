...
import hopper_utils
import plotting
from hopper_soft_actor_critic import HopperSoftActorCritic
...
def collect_avg_obs(env, policies, T, n=100):
    ...
    for iteration in range(n):        
        env.reset()
        obs = hopper_utils.get_state(env, env.env._get_obs())
       
        for t in range(T):
            action = np.zeros(shape=(1,hopper_utils.action_dim))
            ...
            data.append(obs[:29])
            obs = hopper_utils.get_state(env, obs)
...
def compute_states_visited_xy(env, policies, T, n, norm=[],
    ...
    for it in range(N): 
        p_xy = np.zeros(shape=(tuple(hopper_utils.num_states_2d))) 
        ...
        for iteration in range(n):
            ...
            if len(initial_state) > 0:
                qpos = initial_state[:len(hopper_utils.qpos)]
                qvel = initial_state[len(hopper_utils.qpos):]
            ...
            for t in range(T):
                ...
                if p_xy[tuple(hopper_utils.discretize_state_2d(obs, norm, env))]  == 0:
                    cumulative_states_visited_xy += 1
                
                step = iteration*T + t
                states_visited_xy[step] += cumulative_states_visited_xy
                p_xy[tuple(hopper_utils.discretize_state_2d(obs, norm, env))] += 1
...
def execute_one_rollout(policies, weights, env, start_obs, T, data, norm, wrapped=False):
    ...
    for t in range(T):
        ...
        # Count the cumulative number of new states visited as a function of t.
        obs, _, done, _ = env.step(action)
        obs = hopper_utils.get_state(env, obs, wrapped)

        # if this is the first time you are seeing this state, increment.
        if p[tuple(hopper_utils.discretize_state(obs, norm, env))] == 0:
        ...
        p[tuple(hopper_utils.discretize_state(obs, norm, env))] += 1
        p_xy[tuple(hopper_utils.discretize_state_2d(obs, norm, env))] += 1
...
def execute_average_policy(env, policies, T, weights,
    ...
    for iteration in range(n):
        ...
        if render and iteration == 0:
            ...
            obs = hopper_utils.get_state(wrapped_env, wrapped_env.unwrapped._get_obs(), wrapped=True)
...
def grad_ent(pt):
    ...
    eps = 1/np.sqrt(hopper_utils.total_state_space)
    return 1/(pt + eps)

def init_state(env):    
    env.env.set_state(hopper_utils.qpos, hopper_utils.qvel)
...
def collect_entropy_policies(env, epochs, T, MODEL_DIR=''):
    ...
    running_avg_p = np.zeros(shape=(tuple(hopper_utils.num_states)))
    running_avg_p_xy = np.zeros(shape=(tuple(hopper_utils.num_states_2d)))
    running_avg_ent = 0
    running_avg_ent_xy = 0
    entropy_of_running_avg_p = 0.

    running_avg_p_baseline = np.zeros(shape=(tuple(hopper_utils.num_states)))
    running_avg_p_baseline_xy = np.zeros(shape=(tuple(hopper_utils.num_states_2d)))
    ...
    for t in range(10000):     
        ...
        prebuf.store(hopper_utils.get_state(env, obs))
        if done:
            env.reset()
            done = False
    ...
    reward_fn = np.zeros(shape=(tuple(hopper_utils.num_states)))
    ...
    for i in range(epochs):
        ...
        sac = HopperSoftActorCritic(lambda : gym.make(args.env), reward_fn=reward_fn, xid=i+1,
        ...
        if args.autoencode:
            ...
            hopper_utils.learn_encoding(train, test)
